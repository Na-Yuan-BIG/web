...
function cmhtPlot(chr,snp,bp,cp,sp,cgwas_status){
    ...
    if( cgwas_first_stat == "example"){
        selectano(19 ,45415640);
        barplot("rs445925");
    };
    //插入建议线段
    var xianzhu = document.getElementById("gt").value;
    var jianyi = document.getElementById("st").value;
    if(xianzhu ==""){xianzhu = 5e-8};
    if(jianyi == ""){jianyi = 1e-6};
    var lineValues = [xianzhu,jianyi];
    var lineColors = ["red" , "blue"];
    for(var i = 0; i<lineValues.length ;i++){
        ...
        if(line<=5e-8){                             
            //线的坐标
            var upLocation = [padding,Yup2Scale(linep), 0.5*width+padding,Yup2Scale(linep)];
            var downLocation = [padding,Ydown2Scale(linep), 0.5*width+padding,Ydown2Scale(linep)]
        }else{              
            //线的坐标
            var upLocation = [padding,Yup1Scale(linep), 0.5*width+padding,Yup1Scale(linep)];
            var downLocation = [padding,Ydown1Scale(linep), 0.5*width+padding,Ydown1Scale(linep)]   
        };              
        //坐标轴上方
        svg.append("line")
...
function insertChromsome(){
    ...
    svg.selectAll("ellipse")
        ...
        .attr("font-size", "10px")
        .attr("fill", "black")
        .attr("transform", "translate("+0.8*width+","+1.5*padding+")")
        .attr("id" , "text_all");
    //add phenotypes
    //phenotype text
    svg.selectAll("ellipse")
        ...
        .attr("font-size", "10px")
        .attr("fill", "black")
        .attr("transform", "translate("+0.9*width+","+1.5*padding+")")
        .attr("id" , "text_phe");
    //*********************
    //---------------------
    //phenotype button
...
function Changechr(){
    ...
    for(var j=0;j<loci.length;j++){
        ...
        svg.selectAll("ellipse")
            ...
            .attr("cx", widthScale(bp[loci[j]]))
            .attr("cy", function(d , i){
                return 3/2*height+40+12+i*12-5;
            })
            .attr("r", 6)
            .attr("fill" , function(d){
                return phenoColors[Phes.indexOf(d)];

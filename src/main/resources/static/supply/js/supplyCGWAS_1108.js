...
function barplot(rsid){
	...
	barset = svg.selectAll("rect")
			...
			.attr("transform", "translate("+0.45*width+",0)")
			.attr("id" , "bars");
	 //绘制折线图
		line_generator=d3.line()
			...
			.y(function(d){return bar_yScale(d)});
                         ...
                         //.interpolate("linear")
	var g=svg.append("g").attr("transform", "translate("+0.45*width+",0)");
	...
	svg.append("g")
	.attr("class", "axis")
	.attr("transform", "translate("+(0.45*width+padding)+",0)")
	.call(bar_yAxis)
	.attr("id" , "bar_yAxis");
};
function bar_file(){
